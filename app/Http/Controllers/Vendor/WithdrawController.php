...
class WithdrawController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $fines = Fines::where('user_id','=',Auth::guard('web')->user()->id)->where('is_collected','=','0');
        
        if($request->amount > 0 || $request->amount > $fines->sum('fine')){
            ...
            if ($from->current_balance >= $amount){
                ...
                $finalamount = $finalamount - $fines->sum('fine');
                ...
                if($fines !=null){
                    $ids = $fines->pluck('id');
                    $fines = Fines::whereIn('id',$ids->toArray())->update(['is_collected'=>'1']);    
                }
        ...
        $msg = $request->amount > $fines ?"Please Enter a valid amount the fines on you is $fines": 'Please enter a valid amount.';
