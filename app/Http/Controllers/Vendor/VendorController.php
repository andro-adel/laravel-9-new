...
class VendorController extends Controller
{
    ...
    {
        $user = Auth::user();
        if(Auth::guard('web')->user()->Is_Verfiy()){
    ...
    //***POST Request
     public function contract(Request $request, $id){
        ...
        $rules = ['contract_img'  => 'required|mimes:jpeg,jpg,png,svg,pdf',];
        
        $massage = $this->get_massge_contract();
        ...
        $name = time().$file->getClientOriginalName();
        $stauts = $file->move('assets/images/attachments',$name);
        $user = User::find($id);
        $user->contract_img = $name;
        $user->update();
        
        return redirect()->back()->with(['done'=>'تم رفع الصورة و بانتظار موافقه اﻻداراه']);
    }
    //*** GET Request
    ...
    public function get_massge_contract(){
        return [
            'contract_img.required'=>'صورة العقد حقل مطلوب',
             'contract_img.mime'=>'يجب ان تكون صغيه العقد مثل pdf,jpg,jpeg,png,svg',
        ];
    }
