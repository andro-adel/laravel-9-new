...
//                $input['childcategory_id'] = $request->childcategory_id;
    ...
    public function store(Request $request)
    {
        ...
        //--- Logic Section
           ...
           if (empty($attrArr)) {
            ...
            // Save Data
            $check_verfiy = $user->verifies;
            if(isset($user->verifies[0]->id) && !empty($user->verifies)){
                $input['is_official'] ='1';
            }
            $data->fill($input)->save();
            ...
            // Set Photo
            $resizedImage = Image::make(public_path().'/assets/images/products/'.$prod->photo)->resize(1600, 1600, function ($c) {
                $c->aspectRatio();
            });
        ...
        if ($files = $request->file('gallery')){
            ...
            $gallery = new Gallery;
            $name = time().$file->getClientOriginalName();
            $img = Image::make($file->getRealPath())->resize(1600, null, function ($constraint) {
    ...
    public function edit($id)
    {
        ...
        if($data->type == 'Digital')
            return view('vendor.product.edit.digital',compact('cats','data','data_ar','sign'));
        elseif($data->type == 'License')
            return view('vendor.product.edit.license',compact('cats','data','data_ar','sign'));
        else
            return view('vendor.product.edit.physical',compact('cats','data','data_ar','sign'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if($data_ar == null){
            ...
            $data = ['name_ar'=>$request->name_ar ,'details_ar'=>$request->details_ar,'id'=>$id];
            $product_ar->create($data);
            $product_ar->save();
        }
        if($request->name_ar &&$request->details_ar&& $data_ar != null){
