...
// -------------------------------- HOME PAGE SECTION ----------------------------------------
    ...
    public function index(Request $request)
    {
        ...
        for($i=0; $i < $count;$i++){
            if($i <= 6){
                echo $page[$i]->title.'7 <br>';
                if($i == 6)
                    echo '<hr>';
            }
            if($i > 6 && $i <= 13){
                echo $page[$i]->title.'14 <br>';
                if($i == 13)
                    echo '<hr>';
            }
            if($i > 13 && $i <= 20){
                echo$page[$i]->title.' 20 <br>';
            }
        }*/
        if (Session::get('language') == null) {
            Session::put('language', Language::all()->where('is_default','=','1')->first()->id);
        }
        ...
        $selectable = ['id','is_official','user_id','name','slug','features','colors','thumbnail','price','previous_price','attributes','size','size_price','discount_date'];
        $sliders = DB::table('sliders')->get();

        if($lang == 2 || $lang == null){
            $top_small_banners = DB::table('banners')->select('photo_ar','link')->where('type','=','TopSmall')->get();
        }
        else{
        ...
        if($lang == 1 && $lang !=null){
            $feature_products =  Product::where('featured','=',1)->where('status','=',1)->where('products.is_approved','=','1')->select($selectable)->orderBy('id','desc')->take(10)->get();
        }
        else{

            //TODO:Make A Join Query Between products's And Product_Ar
            $feature_products = DB::table('products')
                ->join('product_ars','product_ars.id','=','products.id')
                ->select('products.*', 'product_ars.name_ar', 'product_ars.details_ar')
    ...
    public function extraIndex()
    {
        ...
        if($lang == 2 || $lang == null){
            $bottom_small_banners = DB::table('banners')->select(DB::raw('photo_ar as photo'),'link','id')->where('type','=','BottomSmall')->get();
        }
        else{
        ...
        if($lang == 2 || $lang == null){
            ...
            $large_banners = DB::table('banners')->
            select(DB::raw('photo_ar as photo'),'link','id')
                ->where('type','=','Large')
                ->get();
        }
        else{
        ...
        if($lang == 2 || $lang == null){
            ...
            $discount_products =  Product::where('is_discount','=',1)
                ->where('status','=',1)
                ->where('products.is_approved','=','1')->orderBy('id','desc')->take(8)->get();
        }

        if($lang == 2 ||$lang == null ){
            //TODO:Make A Join Query Between products's And Product_Ar
            $best_products = DB::table('products')
                ->join('product_ars','product_ars.id','=','products.id')
                ->select('products.*', 'product_ars.name_ar', 'product_ars.details_ar')
            ...
            $best_products = Product::where('best','=',1)
                ->where('products.is_approved','=','1')
                ->where('status','=',1)->select($selectable)->orderBy('id','desc')->take(6)->get();
        }


        if($lang == 2 ||$lang == null ){
            //TODO:Make A Join Query Between products's And Product_Ar
            $abrod_products = DB::table('products')
                ->join('product_ars','product_ars.id','=','products.id')
                ->select('products.*', 'product_ars.name_ar', 'product_ars.details_ar')
                ->where('products.is_abroad','=',1)->where('status','=',1)->where('products.is_approved','=','1')->orderBy('id','desc')->take(6)
                ->get();
        }
        else{
            $abrod_products = Product::where('is_abroad','=',1)->where('status','=',1)->where('products.is_approved','=','1')->select($selectable)->orderBy('id','desc')->take(6)->get();
        }
        ...
        if($lang == 2 || $lang == null ){
            ...
            $top_products = DB::table('products')
                ->join('product_ars','product_ars.id','=','products.id')
                ->select('products.*', 'product_ars.name_ar', 'product_ars.details_ar')
                ->where('products.top','=',1)
                ->where('status','=',1)
                ->where('products.is_approved','=','1')
                ->orderBy('id','desc')->take(8)->get();
        }
        else{
            $top_products = Product::where('top','=',1)->where('status','=',1)->where('products.is_approved','=','1')->select($selectable)->orderBy('id','desc')->take(8)->get();;
        }

        if($lang == 2 || $lang == null ){

            //TODO:Make A Join Query Between products's And Product_Ar
            $big_products = DB::table('products')
                ->join('product_ars','product_ars.id','=','products.id')
                ->select('products.*', 'product_ars.name_ar', 'product_ars.details_ar')
                ->where('products.big','=',1)
                ->where('status','=',1)
                ->where('products.is_approved','=','1')
                ->orderBy('id','desc')->take(8)
                ->get();
            ...
            $big_products = Product::where('big','=',1)
                ->where('status','=',1)
                ->where('products.is_approved','=','1')
                ->select($selectable)->orderBy('id','desc')->take(6)->get();
        ...
        if($lang == 2 || $lang == null ){
            ...
            $trending_products = DB::table('products')
                ->join('product_ars','product_ars.id','=','products.id')
                ->select('products.*', 'product_ars.name_ar', 'product_ars.details_ar')
                ->where('products.big','=',1)->where('status','=',1)
                ->where('products.is_approved','=','1')
                ->orderBy('id','desc')->take(9)
                ->get();
        }
        else{
            $trending_products =  Product::where('big','=',1)
                ->where('status','=',1)
                ->where('products.is_approved','=','1')
                ->select($selectable)->orderBy('id','desc')->take(9)->get();
        }
        ...
        return view('front.extraindex',compact('ps','services','reviews','large_banners','bottom_small_banners','abrod_products','best_products','top_products','hot_products','latest_products','big_products','trending_products','sale_products','discount_products','partners'));
    }

    public function seeall($section_name){
        $lang =\session()->get('language');
        ...
        if($lang == 2 || $lang == null){
            ...
            $data =  Product::where("$section_name",'=',1)
                ->where('status','=',1)
                ->where('products.is_approved','=','1')->orderBy('id','desc')->get();
        }
       return view('front.seeall', ['data'=>$data]);
...
// CURRENCY SECTION
    ...
    public function currency($id)
    {
        ...
        if($id == 10){
            $value = Currency::find($id);
            Session::put('value',$value->vlaue);
        }
        return redirect()->back();
    }
...
// CURRENCY SECTION ENDS
    ...
    public function autosearch($slug)
    {
        if(mb_strlen($slug,'utf-8') > 1){
            $search = ' '.$slug;
            $prods = Product::where('status','=',1)->where('products.is_approved','=','1')->where('name', 'like', '%' . $search . '%')->orWhere('name', 'like', $slug . '%')->take(10)->get()->reject(function($item){
