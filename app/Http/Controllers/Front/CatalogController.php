...
class CatalogController extends Controller
{
    ...
    public function category(Request $request, $slug=null, $slug1=null, $slug2=null)
    {
        ...
        $prods = Product::when($cat, function ($query, $cat) {
            return $query->where('category_id', $cat->id)->where('is_approved','=','1');
        })
            ->when($subcat, function ($query, $subcat) {
                return $query->where('subcategory_id', $subcat->id)->where('is_approved','=','1');
            })
            ->when($childcat, function ($query, $childcat) {
                return $query->where('childcategory_id', $childcat->id)->where('is_approved','=','1');
            })
            ->when($search, function ($query, $search) {
                return $query->whereRaw('MATCH (name) AGAINST (? IN BOOLEAN MODE)' , array($search))->where('is_approved','=','1');
            })
            ->when($minprice, function($query, $minprice) {
                return $query->where('price', '>=', $minprice)->where('is_approved','=','1');
            })
            ->when($maxprice, function($query, $maxprice) {
                return $query->where('price', '<=', $maxprice)->where('is_approved','=','1');
            })
            ->when($sort, function ($query, $sort) {
                if ($sort=='date_desc') {
                    return $query->where('is_approved','=','1')->orderBy('id', 'DESC');
                }
                elseif($sort=='date_asc') {
                    return $query->where('is_approved','=','1')->orderBy('id', 'ASC');
                }
                elseif($sort=='price_desc') {
                    return $query->where('is_approved','=','1')->orderBy('price', 'DESC');
                }
                elseif($sort=='price_asc') {
                    return $query->where('is_approved','=','1')->orderBy('price', 'ASC');
                }
            })
       ...
       $prods = Product::when($cat, function ($query, $cat) {
                    ...
                    if (!empty($chFilters)) {
                        ...
                        foreach ($chFilters as $key => $chFilter) {
                            if ($key == 0) {
                                ...
                                ->join('product_ars','product_ars.id','=','products.id')
                                ->select('products.*', 'product_ars.name_ar', 'product_ars.details_ar')->get();
                            } else {
        ...
        $prods = $prods->where('status', 1)->where('is_approved','=','1')
        ->join('product_ars','product_ars.id','=','products.id')
        ->select('products.*', 'product_ars.name_ar', 'product_ars.details_ar')->get();
    ...
    public function product($slug)
    {
        ...
        $this->code_image();
        if ($lang == 2){
            $productt = Product::select('products.*', 'product_ars.name_ar', 'product_ars.details_ar')->where('is_approved','=','1')
                ->join('product_ars','product_ars.id','=','products.id')
                ->where('products.slug','=',$slug)
                ->firstOrFail();
              
               }
               else{$productt = Product::where('products.slug','=',$slug)->where('is_approved','=','1')
            ->firstOrFail();
        ...
        {
            $vendors = Product::where('status','=',1)->where('is_approved','=','1')->where('user_id','=',$productt->user_id)->take(8)->get();
        }
        else
        {
            $vendors = Product::where('status','=',1)->where('is_approved','=','1')->where('user_id','=',0)->take(8)->get();
        }
    ...
    {
        $product = Product::where('slug','=',$slug)->where('is_approved','=','1')->first();
//        $product->views+=1;
//        $product->update();
