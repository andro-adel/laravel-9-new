...
class ProductController extends Controller
{
    ...
    {
         $datas = Product::where('is_approved','=',0)->orderBy('id','desc')->get();
         ...
         return Datatables::of($datas)
                            ...
                            ->addColumn('approve', function(Product $data) {
                                return '
                                <div class="action-list">
                                    <select class="process select droplinks">
                                        <option data-val="1" value="'. route('admin-prod-approve',['id' => $data->id]).'">
                                            Approve
                                        </option>
                                        <option data-val="0" value="'. route('admin-prod-approve',['id' => $data->id]).'">
                                            Deactivated
                                        </option>
                                    </select>
                                </div>';
                            ...
                            ->addColumn('action', function(Product $data) {
                                ...
                                <div class="godropdown">
                                    ...
                                    <i class="fas fa-edit"></i> Edit</a><a href="javascript" class="set-gallery" data-toggle="modal" data-target="#setgallery"><input type="hidden" value="'.$data->id.'"><i class="fas fa-eye"></i> View Gallery</a><a data-href="' . route('admin-prod-feature',$data->id) . '" class="feature" data-toggle="modal" data-target="#modal2"> <i class="fas fa-star"></i> Highlight</a><a href="javascript:;" data-href="' . route('admin-prod-catalog',['id1' => $data->id, 'id2' => 0]) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i> Remove Catalog</a></div></div>';
                            })
                            ->rawColumns(['name', 'approve', 'action'])
                            ->toJson(); //--- Returning Json Data To Client Side
    }
    ...
    public function createPhysical()
    {
         $cats = Category::where('status','=',1)
                ->join('category_ars','category_ars.id','=','categories.id')
                ->select('categories.*', 'category_ars.name_ar')
                ->get();
        $sign = Currency::where('is_default','=',1)->first();
    ...
    public function store(Request $request)
    {
        ...
        if($request->type == "Physical")
        {
            ...
            /*if(isset($request->shipping_time_check)&&$request->shipping_time_check == 1){
                $rules[]=['ship'=>'required'];
            }
            
            if(isset($request->size_check)&&$request->size_check == 1){
                $rules[]=[
                    'size'=>'required',
                    'size_qty'=>'required',
                    'size_price'=>'required'
            ...
            if(isset($request->color_check)&&$request->color_check == 1){
                $rules[]=['color'=>'required'];
            }
            
            if(isset($request->whole_check)&&$request->whole_check == 1){
                $rules[]=[
                    'whole_sell_qty'=>'required',
                    'whole_sell_discount'=>'required'
                    ];
            }*/
        ...
        // Set Thumbnail
        $img = Image::make(public_path().'/assets/images/products/'.$image_name)->resize(1600, 1600);
        $thumbnail = time().str_random(8).'.jpg';
        $img->save(public_path().'/assets/images/thumbnails/'.$thumbnail);
        ...
        $detail_ar = $request->details_ar;
        $ar = [
            'id'=>$data->id,
            'name_ar'=>$name_ar,
            'details_ar'=>$detail_ar,
            
        ];
        $product_ar = Product_ar::create($ar);
        $product_ar->save();
        //Ar end
        //--- Redirect Section
...
//                $input['childcategory_id'] = $request->childcategory_id;
    ...
    public function edit($id)
    {
        ...
        $cats = Category::all();
        $data = Product::findOrFail($id);
        $data_ar = Product_ar::find($id);
        $sign = Currency::where('is_default','=',1)->first();


        if($data->type == 'Digital')
            return view('admin.product.edit.digital',compact('cats','data','data_ar','sign'));
        elseif($data->type == 'License')
            return view('admin.product.edit.license',compact('cats','data','data_ar','sign'));
        else
            return view('admin.product.edit.physical',compact('cats','data','data_ar','sign'));
    }
    ...
    public function update(Request $request, $id)
    {
      // return $request;
        ...
        if($data_ar == null){
            ...
            $data = ['id'=>$id,'name_ar'=>$request->name_ar ,'details_ar'=>$request->details_ar];
            $product_ar->create($data);
        }
        if(isset($request->name_ar) && $data_ar != null){
            $data_ar->name_ar = $request->name_ar;
            $data_ar->details_ar = $request->details_ar;
            $data_ar->id = $id;
            $data_ar->save();
        }
