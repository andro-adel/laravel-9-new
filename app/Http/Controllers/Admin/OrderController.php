        ...
        if ($data->status == "completed") {
            ...
            if ($input['status'] == 'declined') {
                ...
                foreach ($data->vendororders as $vorder) {
                    ...
                    $product = Product::findOrFail($vorder->product_id);
                    $uprice->current_balance = $uprice->current_balance - ($vorder->price - ((($product->category->commission * $product->price) / 100) * $vorder->qty));
                    $uprice->update();
                }
                $data->update($input);
