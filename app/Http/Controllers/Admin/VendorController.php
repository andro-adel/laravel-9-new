...
public function datatables()
	...
	public function datatables_accept(){
	     $datas = User::where('is_vendor','=',2)->Where('is_verfiy','=','0')->orderBy('id','desc')->get();
	         //--- Integrating This Collection Into Datatables
	         return Datatables::of($datas)
                                ->addColumn('verfiy', function(User $data) {
                                    return '<div class="action-list">
                                                <select class="process select vendor-droplinks ">'.
                                                   
                                                    '<option value="'. route('admin-vendor-request-accept',['id' => $data->id,]).'">Deactivated</option>
                                                    <option value="'. route('admin-vendor-request-accept',['id' => $data->id]).'" >Activated</option>
                                                </select>
                                            </div>';
                                }) 
	                            ->addColumn('action', function(User $data) {
    ...
    {
        return view('admin.vendor.accept');
    }
    ...
    public function accept_request($id){
        ...
        $user->is_verfiy = 1;
        $user->update();
        $msg[0] = 'Vendor Request Accepted Successfully.';
        return response()->json($msg);    
    }
	//*** GET Request
