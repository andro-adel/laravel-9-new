...
class Product extends Model
{
    ...
    protected $fillable = ['user_id','category_id','product_type','affiliate_link','sku', 'subcategory_id', 'childcategory_id', 'attributes', 'name', 'photo', 'size','size_qty','size_price', 'color', 'details','price','previous_price','stock','policy','status', 'views','tags','is_official','is_abroad','featured','best','top','hot','latest','big','trending','sale','features','colors','product_condition','ship','meta_tag','meta_description','youtube','type','file','license','license_qty','link','platform','region','licence_type','measure','discount_date','is_discount','whole_sell_qty','whole_sell_discount','catalog_id','slug'];
    ...
    public function vendorPrice() {
        ...
        $price = $this->price;
        if($this->user_id != 0){
        $price = $this->price + (($this->price*$this->category->commission)/100);//$gs->fixed_commission + ($this->price/100) * $gs->percentage_commission ;
        }
    ...
    public  function setCurrency($price) {
        $gs = Generalsetting::findOrFail(1);
    ...
    if(isset($this->attributes["attributes"])){
        ...
        if($curr->id == 10){
             $price = round(($price) * $curr->value,3);
    ...
    public function showPreviousPrice($price ) {
        $gs = Generalsetting::findOrFail(1);
        ...
        if($this->user_id != 0){
        $price = $this->previous_price + (($this->previous_price*$this->category->commission)/100);//$gs->fixed_commission + ($this->previous_price/100) * $gs->percentage_commission ;
        }
    ...
    public function showName($name) {
        $name = mb_strlen($this->name,'utf-8') > 55 ? mb_substr($this->name,0,55,'utf-8').'...' : $name;
        return $name;
    }


    public function emptyStock(string $stck) {
