...
// **************** ADD TO CART *******************
    ...
    public function add($item, $id, $size,$color, $keys, $values) {
        $size_cost = 0;
        $storedItem = ['qty' => 0,'size_key' => 0, 'size_qty' =>  $item->size_qty,'size_price' => $item->size_price, 'size' => $item->size, 'color' => $item->color, 'stock' => $item->stock, 'price' => $item->price, 'item' => $item,'commission'=> 0,'license' => '', 'dp' => '0','keys' => $keys, 'values' => $values];
        ...
        $val = Generalsetting::all();
        $com_val = $val[0]->commission_val;
        $commission =($item->price*$storedItem['qty'])* ".$com_val";
        $storedItem['commission'] = round($commission,1);
...
// **************** ADD TO CART MULTIPLE *******************
    ...
    public function addnum($item, $id, $qty, $size, $color, $size_qty, $size_price, $size_key, $keys, $values) {
        $size_cost = 0;
        $storedItem = ['qty' => 0,'size_key' => 0, 'size_qty' =>  $item->size_qty,'size_price' => $item->size_price,'commission'=> 0, 'size' => $item->size, 'color' => $item->color, 'stock' => $item->stock, 'price' => $item->price, 'item' => $item, 'license' => '', 'dp' => '0','keys' => $keys, 'values' => $values];
        if($item->type == 'Physical')
        {
            if ($this->items) {
                if (array_key_exists($id.$size.$color.str_replace(str_split(' ,'),'',$values), $this->items)) {
